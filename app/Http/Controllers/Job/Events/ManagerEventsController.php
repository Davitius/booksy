...
class ManagerEventsController extends Controller
{
    public function index(Request $request)
    {
        if ($request->input('pers') == '' && Auth::user()->staffstatus == 'Manager') {
            ...
            $ActiveRes = DB::table('events')->where('barber_id', $barberId)->where('finished', 'falce')->orderBy('start', 'ASC')->get();
            $Reservs = DB::table('events')->where('barber_id', $barberId)->where('finished', 'true')->orderBy('updated_at', 'DESC')->paginate(5);
            ...
            return view('Job.Events.Manager.index', compact('Data', 'personal'));
